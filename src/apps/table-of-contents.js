 ...
 */
export class TableOfContentsCompendium extends Compendium {
  /** @inheritDoc */
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["table-of-contents"],
      template: "modules/materia-dnd/templates/apps/table-of-contents.hbs",
      width: 800,
      height: 950,
      resizable: true,
  ...
  async getData(options) {
    ...
    for ( const entry of documents ) {
      const flags = entry.flags?.["materia-dnd"];
      if ( !flags ) continue;
      ...
      const data = {
        ...
        pages: Array.from(entry.pages).map(({ flags, id, name, sort }) => ({
          id, sort, flags,
          name: flags["materia-dnd"]?.title ?? name,
          entryId: entry.id
        }))
      };
